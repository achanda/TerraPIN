...
public class C2DMReceiver extends BroadcastReceiver{
	...
	private void handleData(Context context, Intent intent) {
		...
		{
			app = new Intent(context, UploadActivity.class);
			String[] arr = message.split(":");
			app.putExtra("to", arr[1]);
			app.putExtra("from", arr[2]);
		}
		else
			app = new Intent(context, DownloadActivity.class);

		// Create a pending intent to call the when the notification is clicked
		PendingIntent pendingIntent = PendingIntent.getActivity(context, -1, app, Intent.FLAG_ACTIVITY_NEW_TASK);
	...
	private void handleRegistration (Context context, Intent intent) {
		...
		if (error != null) {
			...
			// TODO: send GET to web server to unregister device from sending list
